...
lucy::Server::~Server() { close(client_fd); }
...
void lucy::Server::use(Middleware middleware) { middlewareHandler.add(middleware); }

void lucy::Server::get(const std::string &path, Handler handler) { trie.insert("GET", path, handler); }
